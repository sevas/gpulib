...
struct gpu_ops_t
{
  ...
  int32_t smp_first;
  int32_t smp_count;
  uint32_t * _Nullable tex;
  uint32_t * _Nullable smp;
  uint32_t vert;
  uint32_t frag;
  uint32_t ppo;
  uint32_t mode;
  int32_t cmd_count;
  struct gpu_cmd_t * _Nullable cmd;
};
...
static inline void gpu_check_exts(
    int32_t extensions_count, const char * _Nonnull * _Nonnull extensions)
{
  for (ptrdiff_t i = 0; i < extensions_count; ++i)
  {
...
static inline uint32_t gpu_window(
    const char * _Nonnull window_title, int32_t window_width,
    int32_t window_height, int32_t msaa_samples, uint32_t sdl_init_flags,
    uint32_t sdl_window_flags, SDL_Window * _Nullable * _Nonnull sdl_window,
    void * _Nullable * _Nullable sdl_glcontext)
{
  ...
  *sdl_window = SDL_CreateWindow(
  ...
  void (*_Nonnull glCreateVertexArrays)(int32_t, uint32_t * _Nullable) =
      SDL_GL_GetProcAddress("glCreateVertexArrays");
  void (*_Nonnull glBindVertexArray)(uint32_t) =
      SDL_GL_GetProcAddress("glBindVertexArray");
  void (*_Nonnull glBlendFunc)(uint32_t, uint32_t) =
...
static inline void * _Nullable gpu_malloc(ptrdiff_t bytes)
{
  uint32_t mem_id = 0;
  glCreateBuffers(1, &mem_id);

  ptrdiff_t size = 4 + bytes;
...
static inline uint32_t gpu_cast(
    void * _Nonnull gpu_mem_ptr, enum gpu_tex_mem_format_t format,
    ptrdiff_t bytes_first, ptrdiff_t bytes_count)
{
  uint32_t tex_id = 0;
...
static inline uint32_t gpu_malloc_tex(
    bool is_cubemap, enum gpu_tex_format_t format, int32_t width,
    int32_t height, int32_t layer_count, int32_t mipmap_count)
{
  uint32_t tex_id = 0;
  glCreateTextures(is_cubemap ? 36873 : 35866, 1, &tex_id);

  glTextureStorage3D(
      tex_id, mipmap_count, format, width, height,
...
static inline uint32_t gpu_malloc_msi(
    enum gpu_tex_format_t format, int32_t width, int32_t height,
    int32_t layer_count, int32_t msaa_samples)
{
  uint32_t tex_id = 0;
  glCreateTextures(37122, 1, &tex_id);

  glTextureStorage3DMultisample(
      tex_id, msaa_samples, format, width, height, layer_count, 1);
...
static inline uint32_t gpu_cast_tex(
    bool is_cubemap, uint32_t tex_id, enum gpu_tex_format_t format,
    int32_t layer_first, int32_t layer_count, int32_t mipmap_first,
    int32_t mipmap_count)
{
  ...
  glTextureView(
      new_tex_id, is_cubemap ? 36873 : 35866, tex_id, format, mipmap_first,
      mipmap_count, layer_first * (is_cubemap ? 6 : 1),
      layer_count * (is_cubemap ? 6 : 1));
...
static inline void gpu_bmp_img(
    uint32_t tex_id, int32_t width, int32_t height, int32_t layer_count,
    const char * _Nonnull * _Nonnull bmp_filepaths)
{
  for (int32_t i = 0; i < layer_count; ++i)
  {
    ...
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i, width, height, 1, gpu_bgr_t, gpu_u8_t,
          bmp->pixels);
      SDL_FreeSurface(bmp);
    }
  }
...
static inline void gpu_bmp_cbm(
    uint32_t tex_id, int32_t width, int32_t height, int32_t layer_count,
    const char * _Nonnull * _Nonnull pos_x_bmp_filepaths,
    const char * _Nonnull * _Nonnull neg_x_bmp_filepaths,
    const char * _Nonnull * _Nonnull pos_y_bmp_filepaths,
    const char * _Nonnull * _Nonnull neg_y_bmp_filepaths,
    const char * _Nonnull * _Nonnull pos_z_bmp_filepaths,
    const char * _Nonnull * _Nonnull neg_z_bmp_filepaths)
{
  for (int32_t i = 0; i < layer_count; ++i)
  {
    ...
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i * 6 + 0, width, height, 1, gpu_bgr_t, gpu_u8_t,
          pos_x_bmp->pixels);
      SDL_FreeSurface(pos_x_bmp);
    }
    if (neg_x_bmp)
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i * 6 + 1, width, height, 1, gpu_bgr_t, gpu_u8_t,
          neg_x_bmp->pixels);
      SDL_FreeSurface(neg_x_bmp);
    }
    if (pos_y_bmp)
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i * 6 + 2, width, height, 1, gpu_bgr_t, gpu_u8_t,
          pos_y_bmp->pixels);
      SDL_FreeSurface(pos_y_bmp);
    }
    if (neg_y_bmp)
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i * 6 + 3, width, height, 1, gpu_bgr_t, gpu_u8_t,
          neg_y_bmp->pixels);
      SDL_FreeSurface(neg_y_bmp);
    }
    if (pos_z_bmp)
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i * 6 + 4, width, height, 1, gpu_bgr_t, gpu_u8_t,
          pos_z_bmp->pixels);
      SDL_FreeSurface(pos_z_bmp);
    }
    if (neg_z_bmp)
    {
      glTextureSubImage3D(
          tex_id, 0, 0, 0, i * 6 + 5, width, height, 1, gpu_bgr_t, gpu_u8_t,
          neg_z_bmp->pixels);
      SDL_FreeSurface(neg_z_bmp);
    }
  }
}

static inline uint32_t gpu_smp(
    int32_t max_anisotropy, enum gpu_smp_filter_t min_filter,
    enum gpu_smp_filter_t mag_filter, enum gpu_smp_wrapping_t wrapping)
{
  uint32_t smp_id = 0;
...
static inline uint32_t gpu_pro(
    enum gpu_shader_t shader_type, const char * _Nonnull shader_string,
    int32_t feedback_count, const char * _Nullable * _Nullable feedback_names)
{
...
static inline uint32_t gpu_pro_file(
    enum gpu_shader_t shader_type, const char * _Nonnull shader_filepath,
    int32_t feedback_count, const char * _Nullable * _Nullable feedback_names)
{
...
#define gpu_vec2(program, location, count, value) glProgramUniform2fv(program, location, count, value)
#define gpu_vec3(program, location, count, value) glProgramUniform3fv(program, location, count, value)
#define gpu_vec4(program, location, count, value) glProgramUniform4fv(program, location, count, value)
// clang-format on
...
static inline uint32_t gpu_ppo(uint32_t vert_pro_id, uint32_t frag_pro_id)
{
  ...
  if (vert_pro_id)
    glUseProgramStages(ppo_id, 1, vert_pro_id);
  if (frag_pro_id)
    glUseProgramStages(ppo_id, 2, frag_pro_id);

  return ppo_id;
}

static inline uint32_t gpu_fbo(
    uint32_t color_tex_id_0, int32_t color_tex_layer_0, uint32_t color_tex_id_1,
    int32_t color_tex_layer_1, uint32_t color_tex_id_2,
    int32_t color_tex_layer_2, uint32_t color_tex_id_3,
    int32_t color_tex_layer_3, uint32_t depth_tex_id_0,
    int32_t depth_tex_layer_0)
{
  uint32_t fbo_id = 0;
  glCreateFramebuffers(1, &fbo_id);

  glNamedFramebufferTextureLayer(
      fbo_id, 36064 + 0, color_tex_id_0, 0, color_tex_layer_0);
  glNamedFramebufferTextureLayer(
      fbo_id, 36064 + 1, color_tex_id_1, 0, color_tex_layer_1);
  glNamedFramebufferTextureLayer(
      fbo_id, 36064 + 2, color_tex_id_2, 0, color_tex_layer_2);
  glNamedFramebufferTextureLayer(
      fbo_id, 36064 + 3, color_tex_id_3, 0, color_tex_layer_3);
  glNamedFramebufferTextureLayer(
      fbo_id, 36096 + 0, depth_tex_id_0, 0, depth_tex_layer_0);

  int32_t attachments[4];
...
static inline uint32_t gpu_xfb(
    void * _Nullable gpu_mem_ptr_0, ptrdiff_t mem_0_bytes_first,
    ptrdiff_t mem_0_bytes_count, void * _Nullable gpu_mem_ptr_1,
    ptrdiff_t mem_1_bytes_first, ptrdiff_t mem_1_bytes_count,
    void * _Nullable gpu_mem_ptr_2, ptrdiff_t mem_2_bytes_first,
    ptrdiff_t mem_2_bytes_count, void * _Nullable gpu_mem_ptr_3,
    ptrdiff_t mem_3_bytes_first, ptrdiff_t mem_3_bytes_count)
{
  uint32_t xfb_id = 0;
  ...
  if (mem_0_id)
    glTransformFeedbackBufferRange(
        xfb_id, 0, mem_0_id, 4 + mem_0_bytes_first, mem_0_bytes_count);
  if (mem_1_id)
    glTransformFeedbackBufferRange(
        xfb_id, 1, mem_1_id, 4 + mem_1_bytes_first, mem_1_bytes_count);
  if (mem_2_id)
    glTransformFeedbackBufferRange(
        xfb_id, 2, mem_2_id, 4 + mem_2_bytes_first, mem_2_bytes_count);
  if (mem_3_id)
    glTransformFeedbackBufferRange(
        xfb_id, 3, mem_3_id, 4 + mem_3_bytes_first, mem_3_bytes_count);
...
static inline void
gpu_draw(int32_t gpu_ops_count, const struct gpu_ops_t * _Nonnull gpu_ops)
{
  __auto_type submit = glDrawArraysInstancedBaseInstance;

  int32_t prev_id = 0;
  int32_t prev_tex_first = 0;
  int32_t prev_tex_count = 0;
  int32_t prev_smp_first = 0;
  int32_t prev_smp_count = 0;
  uint32_t * prev_tex = NULL;
  uint32_t * prev_smp = NULL;
  uint32_t prev_vert = 0;
  uint32_t prev_frag = 0;
  uint32_t prev_ppo = 0;

  for (ptrdiff_t i = 0; i < gpu_ops_count; ++i)
  {
    struct gpu_ops_t ops = gpu_ops[i];

    if (ops.cmd == NULL)
      continue;

    if (ops.tex != NULL &&
        (ops.tex_first != prev_tex_first || ops.tex_count != prev_tex_count ||
         ops.tex != prev_tex))
      glBindTextures(ops.tex_first, ops.tex_count, ops.tex);

    if (ops.smp != NULL &&
        (ops.smp_first != prev_smp_first || ops.smp_count != prev_smp_count ||
         ops.smp != prev_smp))
      glBindSamplers(ops.smp_first, ops.smp_count, ops.smp);

    if (ops.vert != 0 && (ops.vert != prev_vert || ops.id != prev_id))
      glProgramUniform1iv(ops.vert, 0, 1, &ops.id);

    if (ops.frag != 0 && (ops.frag != prev_frag || ops.id != prev_id))
      glProgramUniform1iv(ops.frag, 0, 1, &ops.id);

    if (ops.ppo != 0 && ops.ppo != prev_ppo)
      glBindProgramPipeline(ops.ppo);

    for (ptrdiff_t i = 0; i < ops.cmd_count; ++i)
    {
      struct gpu_cmd_t cmd = ops.cmd[i];
      submit(
          ops.mode, cmd.first, cmd.count, cmd.instance_count,
          cmd.instance_first);
    }

    prev_id = ops.id;
    prev_tex_first = ops.tex_first;
    prev_tex_count = ops.tex_count;
    prev_smp_first = ops.smp_first;
    prev_smp_count = ops.smp_count;
    prev_tex = ops.tex;
    prev_smp = ops.smp;
    prev_vert = ops.vert;
    prev_frag = ops.frag;
...
static inline void
gpu_draw_xfb(int32_t gpu_ops_count, const struct gpu_ops_t * _Nonnull gpu_ops)
{
  __auto_type submit = glDrawArraysInstancedBaseInstance;
  ...
  for (ptrdiff_t i = 0; i < gpu_ops_count; ++i)
  {
    ...
    glBeginTransformFeedback(ops.mode);
    for (ptrdiff_t i = 0; i < ops.cmd_count; ++i)
    {
      struct gpu_cmd_t cmd = ops.cmd[i];
      submit(
...
static inline void gpu_blit(
    uint32_t source_fbo_id, int32_t source_color_id, int32_t source_x,
    int32_t source_y, int32_t source_width, int32_t source_height,
    uint32_t target_fbo_id, int32_t target_color_id, int32_t target_x,
    int32_t target_y, int32_t target_width, int32_t target_height)
{
  ...
  glBlitNamedFramebuffer(
      source_fbo_id, target_fbo_id, source_x, source_y, source_width,
      source_height, target_x, target_y, target_width, target_height, 16384,
      9728);
}

static inline void gpu_blit_to_screen(
    uint32_t fbo_id, int32_t color_id, int32_t width, int32_t height)
{
  glNamedFramebufferReadBuffer(fbo_id, 36064 + color_id);
  glNamedFramebufferDrawBuffer(0, 1029);

  glBlitNamedFramebuffer(
      fbo_id, 0, 0, 0, width, height, 0, 0, width, height, 16384, 9728);
}

#define gpu_clear() glClear(16640)
#define gpu_swap(sdl_window)                                                   \
  {                                                                            \

...
layout(location = 3) uniform int show_pass;

layout(location = 0) smooth in float flogz;
layout(location = 1) smooth in vec3 pos;
layout(location = 2) smooth in vec3 nor;
layout(location = 3) smooth in vec2 uv;
...
void main()
{
  ...
  fbo_color =
      mix((diffuse * 0.4) + (colours * 0.6), reflect,
          dot(nor, normalize(pos - cam_pos)) * 0.5 + 0.5);
